...
def get_profile_id_at_bootgoddb(cart_id):
    ...
    f = urllib.urlopen("http://bootgod.dyndns.org:7777/search.php?" +
                       urllib.urlencode({"kwtype": 'game',
                                         "keywords": cart_id})).read()
...
def get_all_info_from_profile(profile_id):
    f = urllib.urlopen("http://bootgod.dyndns.org:7777/profile.php?" +
                       urllib.urlencode({"id": profile_id})).read()

    doc = lxml.html.document_fromstring(f)

    e = doc.xpath(
        '//table[not(@class)]//th[text()="Catalog ID"]/../..')[0][:-1]
    ...
    d['Romanized Title'] = doc.xpath('//td[@class="headingmain"]/text()')[0].\
        strip()
    return d
        ...
        if cached(cached_db, cart['catalog_id'], cart['original_title']):
            ...
            if tmp:
                ...
                if "Release Date" in entry:
                    cart["released"] = correct_date_bootgod(
                        entry["Release Date"])
                ...
                if "Romanized Title" in entry:
                    ...
                    cart["short_title"] = entry["Romanized Title"].\
                ...
                pprint(cart)
                entry = search_for_release_date_and_publisher(
                    fami_db,
                    cart["catalog_id"])
                ...
                if "name" in entry:
                    ...
                    cart["short_title"] = entry["name"].split(':')[0].\
                        split('!')[0].strip()
                if "publisher" in entry:
                    cart["publisher"] = entry["publisher"]
            db[str(uuid4())] = cart
